...
def initialize_model(config, ckpt):
    ...
    #check model
    #with open('model_beforeload.txt', 'w') as f:
    #    print(model, file=f)#
...
def get_edgemap(sampler,uc_full,model,batch,device,model_LGP,cond):
    ...
    pred_edge_map = pred_edge_map#*(batch["mask"].cpu()<0.5)#without the mask
...
def resizer(image,mask,sketch):
    ...
    if (bottom+top)/2 < 256:
        ...
        bottom = max(0, torch.div((bottom +top), 2, rounding_mode='floor') - length)
        top = bottom + 2 * length - 1
    else:
        top = min(511, torch.div((bottom + top), 2, rounding_mode='floor') + length)
        bottom = top - 2 * length + 1

    if (left+right)/2 < 256:
        # Top padding, assuming we want at least `vertical_length/2` padding
        left = max(0, torch.div((left + right), 2, rounding_mode='floor') - length)
        right = left + 2 * length - 1
    else:
        right = min(511, torch.div((left + right), 2, rounding_mode='floor') + length)
        left = right - 2 * length + 1
    
    position = [bottom,left]
    #print(position,top,right)##

    image_small = image[:, :, bottom:top+1, left:right+1]
    mask_small = mask[:, :, bottom:top+1, left:right+1]
    sketch_small = sketch[:, :, bottom:top+1, left:right+1]
...
def mix_img(image, image_small, mask, length, position):
    ...
    image_small[:, :, bottom:bottom+length, left:left+length] = resized
    # mix
...
def inpaint(sampler,model_LGP, image, mask, prompt, seed, scale, sketch_scale, ddim_steps, middle_number, end_number, num_samples=1, w=512, h=512):
    ...
    with torch.autocast("cuda"):
        ...
        edge_map.save(f"/home/tianle/Desktop/stable-diffusion/experiment/sketch_scale_output/edge/edge_scale{sketch_scale}_number{seed}.jpg")##
...
def run():
    ...
    output_dir = "/home/tianle/Desktop/stable-diffusion/experiment/sketch_end_number_output/"
    ...
    scale = st.slider("Scale", min_value=0.0, max_value=30.0, value=0.0, step=0.1)
    #sketch_scale = st.slider("Sketch scale(X1000)",min_value=0,max_value=500, value=15, step = 1)
    #sketch_scale = sketch_scale/1000
    #middle_number = st.slider("Middle nmuber",min_value=0,max_value=1000,value=650, step = 1)
    ddim_steps = st.slider("DDIM Steps", min_value=0, max_value=50, value=50, step=1)

    sketch_scale = 30.0
    middle_number = 501#end number 
    loss = 0
    masks = []
    for i in range(99):
        mask = generate_square_mask(image_size=(512, 512),square_size=np.random.randint(100, 150),d_x= np.random.randint(-50, 50),d_y=np.random.randint(-50, 50))
        masks.append(mask)

    if prompt:
        for num in range(10):
            middle_number = middle_number - 50
            loss = 0
            numb = 0
            for filename in os.listdir(folder_A):
                if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".jpeg"):
                    ...
                    loss = loss + loss_tem
                    for image in result:
                        image.save(f"{output_dir}result/end_number_{middle_number}_number_{numb}.png")

                    # Save the image
                    mask.save(f"{output_dir}mask/end_number_{middle_number}_number_{numb}.png")
                    numb = numb + 1


            with open(f"{output_dir}sketch_end_loss.txt", 'a') as file:
                print(f"{'end_number'}: {middle_number:.1f} {'loss'}:{loss:.3f}")
                file.write(f"{'end_number'}: {middle_number:.1f} {'loss'}:{loss:.3f}\n")
