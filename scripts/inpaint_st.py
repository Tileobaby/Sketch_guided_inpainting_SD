...
def inpaint(sampler,model_LGP, image, mask, prompt, seed, scale, sketch_scale, ddim_steps, middle_number, num_samples=1, w=512, h=512):
    ...
    with torch.autocast("cuda"):
        ...
        #generate the window
        st.write("Sketch")##added
        st.caption("Draw a sketch to inpaint, then click the 'Send to Streamlit' button (bottom left, with an arrow on it).")
        sketch_result = st_canvas(
            fill_color=fill_color,
            stroke_width=stroke_width,
            stroke_color=stroke_color,
            background_color=bg_color,
            background_image=edge_map,###
        ...
        if sketch_result:
            sketch = sketch_result.image_data
            #print(type(sketch),sketch.shape)# 
            sketch = sketch[:, :, :3]
            if sketch.sum() > 0:

                #sketch = np.array(sketch)
                #print(sketch.sum(),sketch.shape)
                #sketch = gaussian_filter(sketch, sigma=2)
                
                #sketch = sketch[None].transpose(0, 3, 1, 2)                
                #sketch = sketch * np.array(batch["mask"].cpu()) + pred_edge_map

                sketch = pred_edge_map#

                sketch = torch.from_numpy(sketch).to(dtype=torch.float32)
...
def run():
    ...
    lgp_path = '/home/tianle/Desktop/stable-diffusion/LGP_real_5k_3p_5.pt'
    ...
    if image:
        ...
        sketch_scale = st.slider("Sketch scale",min_value=0,max_value=100, value=10, step = 1)
        middle_number = st.slider("Middle nmuber",min_value=0,max_value=1000,value=700, step = 1)
