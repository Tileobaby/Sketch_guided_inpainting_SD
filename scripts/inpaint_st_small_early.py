...
def inpaint(sampler,model_LGP, image, mask, prompt, seed, scale, sketch_scale, ddim_steps, middle_number, num_samples=1, w=512, h=512):
    ...
    with torch.autocast("cuda"):
        ...
        if sketch_result:
            sketch = sketch_result.image_data#512,512,4
            if sketch.sum() > 0:
                sketch = Image.fromarray(sketch,mode="RGB")
                sketch = np.array(sketch)
                sketch = sketch[None].transpose(0, 3, 1, 2)
                sketch = sketch + pred_edge_map
                sketch = torch.from_numpy(sketch)
            
                ##end

                ####add the cut and resize part to get the unmasked area
                ...
                samples_cfg_s, intermediates = sampler.sample(
                        model_LGP,
                        sketch_small,##
                        middle_num,
                        image_m,
                ...
                x_samples_ddim_s = model.decode_first_stage(samples_cfg_s)
                
                result = torch.clamp((x_samples_ddim_s+1.0)/2.0,
                                        min=0.0, max=1.0)
                ...
                st.write("Inpainted small one")
                for img in result:
                    st.image(img)

                #resize
                image_m = mix_img(image, x_samples_ddim_s, mask, length, position)
                image_m = image_m.to(device = device)
                image_m = model.get_first_stage_encoding(model.encode_first_stage(image_m))
                #batch_m = batch
                #batch_m['image'] = image_m.to(device=device)
...
def run():
    ...
    if image:
        ...
        middle_number = st.slider("Middle nmuber",min_value=0,max_value=1000,value=500, step = 1)
