...
def run():
    ...
    output_dir = "/home/tianle/Desktop/stable-diffusion/experiment/sketch_scale_output/"
        ...
        for num in range(25):
            ...
            for filename in os.listdir(folder_A):
                if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".jpeg"):
                    ...
                    loss = loss + loss_tem
                    for image in result:
                        image.save(f"{output_dir}result/result_scale_{sketch_scale}_number_{numb}.png")

                    # Save the image
                    mask.save(f"{output_dir}mask/mask_scale_{sketch_scale}_number_{numb}.png")
                    numb = numb + 1


            with open(f"{output_dir}sketch_scale_loss.txt", 'a') as file:
                print(f"{'sketch_scale'}: {sketch_scale:.1f} {'loss'}:{loss:.3f}")
                file.write(f"{'sketch_scale'}: {sketch_scale:.1f} {'loss'}:{loss:.3f}\n")
