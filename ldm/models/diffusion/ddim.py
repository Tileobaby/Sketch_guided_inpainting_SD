...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, model_LGP, sketch, middle_number,image_m, cond, shape,
        ...
        #show the decoded edge 
        decoded_edge = self.model.decode_first_stage(encoded_edge)
        result = torch.clamp((decoded_edge+1.0)/2.0,min=0.0, max=1.0)
        result = result.cpu().numpy().transpose(0,2,3,1)
        result = result*255
        result = result.squeeze(0)
        result = Image.fromarray(result.astype(np.uint8))
        result.save('decoded_sketch.jpg')
    ...
    def p_sample_ddim(self, model_LGP, x, encoded_edge, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., sketch_scale=20,  unconditional_conditioning=None):
        ...
        if t > 200:
            ...
            #for linear sketch_scale
            #sketch_scale = 0.009+(t-700)*sketch_scale

            #alpha = sketch_scale * torch.norm(x_prev - x ,p=2) / torch.norm(gradient_noisy_image, p=2)
            #print(torch.norm(x_prev - x ,p=2) / torch.norm(gradient_noisy_image, p=2))#3000-200
            alpha = sqrt_one_minus_alphas[index]

            x_prev = x_prev - sketch_scale * alpha * gradient_noisy_image
